...
- LMS at https://courses.infralabs.cloud
- CMS at https://studio.courses.infralabs.cloud
- CDN at https://cdn.courses.infralabs.cloud linked to a public read-only S3 bucket named courses-infralabs-global-storage
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.infralabs.cloud
- private vpc access to MySQL instance at mysql.infralabs.cloud
- private vpc access to MongoDB instance at mongodb.infralabs.cloud
- daily data backups archived into a private S3 bucket named prod-infralabs-global-mongodb-backup

The following adminstrative urls were automatically created:

- Kubernetes Dashboard - https://dashboard.k8s.infralabs.cloud
- Kubeapps  - https://kubeapps.k8s.infralabs.cloud
- Grafana - https://grafana.k8s.infralabs.cloud
...
- LMS at https://dev.courses.infralabs.cloud
- CMS at https://studio.dev.courses-infralabs.cloud
- CDN at https://cdn.dev.courses.infralabs.cloud linked to an S3 bucket named dev-infralabs-global-storage
- daily data backups archived into an S3 bucket named dev-infralabs-global-mongodb-backup
...
.. code-block:: shell
  ...
  ssh bastion.courses.infralabs.cloud -i path/to/infralabs-global-live-bastion.pem

  mysql -h mysql.courses.infralabs.cloud -u root -p

  mongo --port 27017 --host mongo.master.courses.infralabs.cloud -u root -p
  mongo --port 27017 --host mongo.reader.courses.infralabs.cloud -u root -p

  redis-cli -h redis.primary.courses.infralabs.cloud -p 6379
...
- `Kubernetes Dashboard <https://kubernetes.io/docs/tasks/access-application-cluster/web-ui-dashboard/>`_ at https://dashboard.k8s.infralabs.cloud. Written by the same team that maintain Kubernetes, Kubernetes Dashboard provides an elegant web UI for monitoring and administering your kubernetes cluster.
- `Kubeapps <https://kubeapps.dev/>`_ at https://kubeapps.k8s.infralabs.cloud. Maintained by VMWare Bitnami, Kubeapps is the easiest way to install popular open source software packages from MySQL and MongoDB to Wordpress and Drupal.
- `Grafana <https://grafana.com/>`_ at https://grafana.k8s.infralabs.cloud/login. Provides an elegant web UI to view time series data gathered by prometheus and metrics-server.
  - user: admin
  - pwd: prom-operator
...
Use `this automated Github Actions workflow <https://github.com/sainusahib/openedx_devops/actions/workflows/tutor_build_image.yml>`_ to build a customized Open edX Docker container based on the latest stable version of Open edX (current nutmeg.2) and
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.courses.infralabs.cloud:3306. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/prod/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.courses.infralabs.cloud:27017 and mongodb.reader.courses.infralabs.cloud. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.courses.infralabs.cloud. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/prod/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
